...
The hand-labelled behavioural test set for the LLM path of the pipeline. Zero-cost tests around the deterministic reconciler live under `tests/`. This directory covers the parts of the pipeline that involve model calls.

## `golden.jsonl`

One JSON object per line. Each row is one `(control × sample × attribute)` triple with the expected verdict and a note explaining the reasoning.

Row shape:

| Field | Type | Purpose |
| --- | --- | --- |
| `id` | string | Stable identifier as `<control>-<sample>-<slug>`. Category-aware slugs (`-testing`, `-remediation`, `-approval`, `-independence`, `-cadence`, `-ownership`) let the scoreboard slice by category. |
...
| `notes` | string | Free-text rationale. Not scored. If a reviewer wants to argue with a label, the reasoning lives here. |
...
Reads `golden.jsonl`, runs the pipeline under each model against the specified control, scores by normalised `attribute_text`, and prints matches and mismatches side by side with the golden verdict and the model's actual verdict.

Cost note: a full multi-model eval run is roughly $3 to $6 depending on cache warmth. Use `--models claude` for cheaper single-model iteration.

## Environment

Needs `ANTHROPIC_API_KEY` at minimum (default model is Claude). Add `OPENAI_API_KEY` or `GOOGLE_API_KEY` only for the multi-model scoreboard.
